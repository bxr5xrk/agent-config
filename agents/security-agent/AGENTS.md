...
Before working, read `/Users/berserk/Work/agent-config/agents/security-agent/SOUL.md` and local `/Users/berserk/Work/agent-config/agents/security-agent/MEMORY.md` when present, then inspect the current project's applicable instructions and domain documents. Follow the canonical `security` workflow at `/Users/berserk/Work/agent-config/skills/personal/security/SKILL.md` and load only the references it routes to for the current task.
